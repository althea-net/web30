...
impl Web3 {
    ...
    pub async fn send_transaction(
        ...
        let gas_limit = if let Some(gl) = gas_limit {
            ...
            let gas = simulated_gas_price_and_limit(our_balance.clone());
    ...
    pub async fn contract_call(
        ...
        let gas = simulated_gas_price_and_limit(our_balance);
...
// This function will navigate all these restrictions in order to give you the
// maximum valid gas possible for any simulated call
fn simulated_gas_price_and_limit(balance: Uint256) -> SimulatedGas {
    let price: Uint256 = 10u8.into();
    let limit = min(25_000_000u128.into(), balance / price.clone());
    SimulatedGas { limit, price }
}
