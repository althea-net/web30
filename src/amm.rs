use clarity::{Address, Uint256, abi::Token, constants::{TT160M1, TT24M1}};
use crate::{client::Web3, jsonrpc::error::Web3Error};

lazy_static! {
    pub static ref UNISWAP_QUOTER_ADDRESS: Address = Address::parse_and_validate("0xb27308f9F90D607463bb33eA1BeBb41C27CE5AB6").unwrap();
    pub static ref DAI_CONTRACT_ADDRESS: Address = Address::parse_and_validate("0x6B175474E89094C44Da98b954EedeAC495271d0F").unwrap();
    pub static ref WETH_CONTRACT_ADDRESS: Address = Address::parse_and_validate("0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2").unwrap();
}
...
impl Web3 {
    ...
    pub async fn get_uniswap_price(
        ...
        if fee_uint24 > *TT24M1 {
            return Err(Web3Error::BadInput("Bad fee input to swap price - value too large for uint24".to_string()))
        }
        if sqrt_price_limit_x96_uint160 > *TT160M1 {
            return Err(Web3Error::BadInput("Bad sqrt_price_limit_x96 input to swap price - value too large for uint160".to_string()))
        }

        let tokens: &[Token] = &[Token::Address(token_in), Token::Address(token_out), Token::Uint(fee_uint24), Token::Uint(amount), Token::Uint(sqrt_price_limit_x96_uint160)];
        debug!("tokens is {:?}", tokens);
        let result = self
            .contract_call(quoter, "quoteExactInputSingle(address,address,uint24,uint256,uint160)",
            &tokens, caller_address, None)
            .await?;
...
fn get_uniswap_price_test() {
    ...
    let amount = Uint256::from(1_000_000_000_000_000_000 as u64);
    let fee = Uint256::from(500 as u16);
    let sqrt_price_limit_x96_uint160 = Uint256::from(0 as u16);

    runner.block_on(async move {
        let price = web3.get_uniswap_price(caller_address.clone(), *WETH_CONTRACT_ADDRESS, *DAI_CONTRACT_ADDRESS,
         fee.clone(), amount.clone(), sqrt_price_limit_x96_uint160.clone(), None).await;
        let weth2dai = price.unwrap();
        debug!("weth->dai price is {}", weth2dai);
        assert!(weth2dai > 0u32.into());
        let price = web3.get_uniswap_price(caller_address.clone(), *DAI_CONTRACT_ADDRESS, *WETH_CONTRACT_ADDRESS,
             fee.clone(), weth2dai, sqrt_price_limit_x96_uint160, None).await;
